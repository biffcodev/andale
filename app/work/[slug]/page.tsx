...
import { enrichWorks, imgUrl, LOCATIONS, mediaAbs, projectGallery, PROJECT_VIDEOS } from "@/lib/content";
...
export default function ProjectPage() {
  ...
  const pid = work.index;
  const g = projectGallery(pid);
  const next = works[(pid + 1) % works.length];
  const videoSrc = PROJECT_VIDEOS[pid % PROJECT_VIDEOS.length];
  ...
  const meta = [
    ...
    { k: t.ui.year, v: work.year },
    { k: t.ui.sector, v: work.sector },
    { k: t.ui.location, v: LOCATIONS[pid] || "Córdoba" },
  ...
  return (
    <main id="maincontent" role="main">
      ...
      <section style={{ position: "relative", height: "100vh", overflow: "hidden" }}>
        <div style={mediaAbs(work.resolvedImg)} />
      ...
      <section style={{ padding: "clamp(80px,14vh,170px) clamp(20px,6vw,110px)", display: "grid", gridTemplateColumns: "repeat(auto-fit,minmax(300px,1fr))", gap: "clamp(40px,6vw,110px)", alignItems: "start" }}>
        <Reveal>
          ...
          <p style={{ marginTop: 22, fontSize: "clamp(20px,1.9vw,30px)", fontWeight: 500, letterSpacing: "-.02em", lineHeight: 1.32, maxWidth: "34ch" }}>{work.challenge}</p>
        </Reveal>
        <Reveal>
          ...
          <p style={{ marginTop: 22, fontSize: "clamp(20px,1.9vw,30px)", fontWeight: 500, letterSpacing: "-.02em", lineHeight: 1.32, maxWidth: "34ch" }}>{work.approach}</p>
        </Reveal>
      </section>

      {/* ---------- FILM ---------- */}
      <div style={{ position: "relative", height: "100vh", overflow: "hidden", background: "#000" }}>
        <video ref={videoRef} muted loop playsInline poster={imgUrl(work.resolvedImg)} style={{ position: "absolute", inset: 0, width: "100%", height: "100%", objectFit: "cover" }}>
        ...
        {!playing ? (
          ...
          >
            <span
              style={{ display: "grid", placeItems: "center", width: "clamp(84px,9vw,120px)", height: "clamp(84px,9vw,120px)", borderRadius: 999, border: "1px solid rgba(255,255,255,.55)", background: "rgba(0,0,0,.28)", backdropFilter: "blur(8px)", WebkitBackdropFilter: "blur(8px)", transition: "transform .4s cubic-bezier(.22,1,.36,1), background .3s" }}
            >
            ...
            <span style={{ textAlign: "center" }}>
              ...
              <span style={{ display: "block", fontSize: "clamp(24px,3vw,44px)", fontWeight: 800, letterSpacing: "-.03em", lineHeight: 1.05, marginTop: 10 }}>{work.title}</span>
            </span>
          </button>
        ) : (
          <button
            ...
            aria-label="Pause"
            className="frostbtn"
            style={{ position: "absolute", right: "clamp(20px,6vw,90px)", bottom: "clamp(38px,7vh,66px)", zIndex: 20, width: 54, height: 54, border: "1px solid rgba(255,255,255,.5)" }}
          >
      ...
      <section style={{ padding: "clamp(80px,14vh,170px) clamp(20px,6vw,110px)", maxWidth: 1100 }}>
        ...
        <Reveal as="p" style={{ marginTop: "clamp(24px,3vh,38px)", fontSize: "clamp(28px,3.6vw,58px)", fontWeight: 600, letterSpacing: "-.025em", lineHeight: 1.14, maxWidth: "22ch", textWrap: "balance" }}>
          {work.outcome}
        </Reveal>
      </section>
      ...
      >
        <div style={{ position: "absolute", inset: 0, display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", gap: 16, color: "var(--fg)", padding: 20 }}>
          ...
          <h2 style={{ fontSize: "clamp(36px,6vw,92px)", fontWeight: 800, letterSpacing: "-.035em", lineHeight: 1, maxWidth: "16ch", textWrap: "balance" }}>{next.title}</h2>
